...
import SECTION_OBJECT from '@salesforce/schema/Section__c';//RRA - Ticket 2344 - 10/03/2024
import CURRENCY_FIELD from '@salesforce/schema/Section__c.Currency__c'; //RRA - Ticket 2344 - 10/03/2024
...
export default class LWC33_MarketSubmission extends LightningElement {
    ...
    @api lstIdRiskCarrierChanged = [];//RRA - Ticket 2304 - 12/02/2025
    isDisplayValueFinancialEnt = true;
    cedCompany;
    uwYear;
    programName;   
    ...
    brokerName;
    reinsurerName;
    riskCarrierName; //RRA - Ticket 2304 - 12/02/2025
    selectedReinsurerId;
    selectedRiskCarrierId; //RRA - Ticket 2304 - 12/02/2025
    selectedIdRequest; //RRA - Ticket 2304 - 12/02/2025
    ...
    @track isDisabledFinancial = false;
    ...
    mapCurrencyLabelCurrencyAPIName; //RRA - Ticket 2344 - 10/03/2025
    ...
    get inputVariables() {
        ...
        //RRA - Ticket 2304 - 12/02/2025
        {
            name: 'RiskCarrierID',
            type: 'String',
            value: this.selectedRequest['RiskCarrier__c']
        },
    ...
    //1417
    handleOnChangeBrokerStatus(event){
        this.brokerStatusValue = event.target.value;
        if (this.portalAccess || this.portalAccess == false){
            console.log('brokerStatusValue == ', this.brokerStatusValue);
            if (this.brokerStatusValue == '2'){
                this.isDisabledFinancial = true;
                this.selectedRequest['FinancialName'] = null;
                this.selectedRequest['FinancialEntity__c'] = null;
            }else if (this.brokerStatusValue == '1'){
                this.isDisabledFinancial = false;
            }
            console.log('isDisabledFinancial == ', this.isDisabledFinancial);
        }
    }
    ...
    getRequestDetails(){
        ...
        .then(result => {
            ...
            if(result.hasOwnProperty('Error') && result.Error){
                ...
                let isBoolean;
                isBoolean = "true" ? sessionStorage.getItem('isValueFinancialEntityEmpty') === "true" : "false" ? sessionStorage.getItem('isValueFinancialEntityEmpty') === "false" : null;
                console.log('isBoolean == ', isBoolean);
                if (result.BrokerStatus__c == '2'){
                        if (this.portalAccess == false){
                            this.template.querySelectorAll('[data-id="' + result.Id + '"]').forEach(currentItem => {
                                currentItem.disabled = true;
                                currentItem.value = null;
                          });
                        }else{
                          //RRA - Ticket 2236 - 24/12/2024
                          if (isBoolean == false){
                            this.isDisabledFinancial = false;
                            this.selectedRequest['FinancialName'] = null;
                          }
                ...
                }else if (result.BrokerStatus__c == '1'){
                    if (this.portalAccess == false){
                        this.template.querySelectorAll('[data-id="' + result.Id + '"]').forEach(currentItem => {
                ...
                if(result['Id'] != undefined || result['Id'] != null){
                    ...
                    if(request['ReinsurerStatus__c'] == 'Timeout' && this.portalAccess == true){
                        ...
                        this.isDisabledFinancial = true;//1417
                        this.isStatusSigned = true;
                    ...
                    //RRA - Ticket 2304 - 12/02/2025 
                    if(request['RiskCarrier__c'] != undefined){
                        this.riskCarrierName = request['Reinsurer__r']['Name'];
                        this.selectedRiskCarrierId = this.selectedRequest['RiskCarrier__c'];
                    }
    ...
    getListRequests(){
        ...
        .then(result => {
            if(result.hasOwnProperty('Error') && result.Error){
                ...
                let mapKeyListCededPremium = result.mapKeyListCededPremium;      
                let lstUpdReq = [];
                let lstCededCurrency = [];
                ...
                if (isDeactivatedProg){
                 ...
                 console.log('mapKeyListCededPremium == ', mapKeyListCededPremium);
                console.log('lstReq == ', lstReq);

                //RRA - Ticket 2344 - 11/03/2025
                for(let key in mapKeyListCededPremium) {
                   let value = mapKeyListCededPremium[key];
                   lstCededCurrency.push({key : key, value : value});
                }        
                let valueCeded = '';
                let lstValueCeded = [];
                 console.log( "lstCededCurrency ", lstCededCurrency);
                for(let i = 0; i < lstReq.length; i++){
                    ...
                    rowReq['SignedShareVal'] = parseFloat(rowReq.SignedShare__c).toFixed(6).replace('.',','); 
                       ...
                       //RRA - Ticket 2344 - 11/03/2025 - Display value on UI
                       for(let i = 0; i < lstCededCurrency.length; i++){                      
                        if (lstCededCurrency[i].key == rowReq.Treaty__c){
                          lstValueCeded = lstCededCurrency[i].value; //+  
                          
                        }
                     }
                   
                    // get value as String and add space between value to be convert in line break from css show-newlines
                    for(let i = 0; i < lstValueCeded.length; i++){                      
                      valueCeded = valueCeded + lstValueCeded[i] + '\n';
                        if (lstValueCeded[i] != undefined){
                           rowReq['cededPremiumVal'] = valueCeded; 
                        }
                    }
                     valueCeded = '';
                    ...
                    console.log('isDisabledFinancial == ', this.isDisabledFinancial);
                    console.log('isStatusTimeout == ', this.isStatusTimeout);

                    if(this.isStatusTimeout == true || this.isDisabledFinancial){//1417
    ...
    handleSaveRequest(event){
        ...
        for(let i = 0; i < this.lstRelatedSigningRequests.length; i ++){
            ...
            if(btnName == 'Send'){
              ...
              console.log('idReqFromChangedRiskCarrier == ', this.selectedIdRequest);
                saveRequests({lstRequests: this.lstRelatedSigningRequests, buttonName:btnName, reinsurerId: this.selectedReinsurerId, isPortal: this.portalAccess, idProgram: this.programId, idReqRiskCarrier : this.selectedIdRequest})//MRA 19/05/23 - Contact Signatory Rebuild ** RRA - Ticket 2304 - 13/02/2025
                .then(result => {
                    if(result.hasOwnProperty('Error') && result.Error){
                        this.dispatchEvent(new ShowToastEvent({title: 'Error', message: result.Error, variant: 'error' }),);
                        this.displaySpinner = false;
    ...
    saveSignatories(){//MRA 19/05/23 - Contact Signatory Rebuild
        console.log('### selectedRiskCarrierId saveSignatories= ' , this.selectedRiskCarrierId) ;   
        saveSignatories({lstContacts: this.signatoriesData,riskCarrierId : this.selectedRiskCarrierId, reinsurerId: this.selectedReinsurerId,idProgram:this.programId}) //RRA - Ticket 2304 - 12/02/2025
        .then(result => { 
            if(result.hasOwnProperty('Error') && result.Error){
                this.dispatchEvent(new ShowToastEvent({title: 'Error', message: result.Error, variant: 'error' }),);
            }
    ...
    getSignatories(){
         console.log('### selectedReinsurerId = ' , this.selectedReinsurerId) ;      
          console.log('### selectedRiskCarrierId getSignatories= ' , this.selectedRiskCarrierId) ;   
        getSignatories({reinsurerId : this.selectedReinsurerId, riskCarrierId : this.selectedRiskCarrierId, programId : this.programId}) //RRA - Ticket 2304 - 12/02/2025
        .then(result => {
            ...
            if(result.hasOwnProperty('Error') && result.Error){
              ...
              console.log('### resultresult = ' , result['Success'])            
    ...
    setSelectedRiskCarrierLookupRecord(event) {
        ...
        this.selectedIdRequest = this.selectedRequest['Id']; //RRA - ticket 2304 - 13/02/2025
        ...
        console.log('### selectedIdRequest = ' + this.selectedIdRequest);
