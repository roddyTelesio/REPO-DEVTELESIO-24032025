...
import QUOTE_TYPE_FIELD from '@salesforce/schema/Request__c.QuoteType__c';  // HRA W-1096 - QuoteType Filter
...
export default class LWC11_TreatyPlacementFilters extends LightningElement{
    ...
    // HRA W-1096 - QuoteType Filter
    @wire(getPicklistValues, { recordTypeId: '$requestRTId', fieldApiName: QUOTE_TYPE_FIELD})
    setQuoteTpePicklistOptions({error, data}) {
        if(data){
            let buffer = [{label:'--All--', value:'--All--'}];
    ...
    hasLoaded; //BBH 30.10.2024 - Dev ReadOnly W-2163:AUDIT - Create a Read only profile for all platform except Works tab
        ...
        if(result.data){
            if(result.data.length == 0){
                ...
                .then(resultProg => {
                    ...
                    if(this.hasLoaded){//BBH 30.10.2024 - Dev ReadOnly W-2163:AUDIT - Create a Read only profile for all platform except Works tab - NO NEED TO PERFORM THIS CHECK ON THE FIRST TIME THE PAGE HAS LOADED
   ...
   getSomeInfoSectionsTreatyPlacement(progId){
    ...
    wiredGetAccountBroker(result){
        ...
        if(result.data) {
            ...
            let brokerOptionFirstList = [{label:'--All--', value:'--All--'}];
            let newBrokerOptionsSecondList = [{label:'--Direct--', value:'--Direct--'}];
            let newBrokerOptions = brokerOptionFirstList.concat(newBrokerOptionsSecondList);
            result.data.forEach(key => {
    ...
    handleChangeBroker(event){
        ...
        if(this.valueBroker == '--All--'){
            fireEvent(this.pageRef, 'valueBroker', 'All');
        //RRA - ticket 1091 - 15072024
        }
        
         if(this.valueBroker == '--Direct--'){ 
            fireEvent(this.pageRef, 'valueBroker', 'direct');
        }
         
         if (this.valueBroker.includes('001')){
    ...
    // HRA W-1096 - QuoteType Filter
    handleChangeQuoteType(event){
        this.valueQuoteType = event.detail.value;
        if(this.valueQuoteType == '--All--'){
            fireEvent(this.pageRef, 'valueQuoteType', null);
        }
        else{
            fireEvent(this.pageRef, 'valueQuoteType', this.valueQuoteType);
        }
    }
