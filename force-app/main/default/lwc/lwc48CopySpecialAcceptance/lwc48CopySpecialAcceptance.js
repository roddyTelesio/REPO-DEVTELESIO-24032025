...
import {label} from './lwc48CopySAUtils';
...
export default class Lwc48CopySpecialAcceptance extends NavigationMixin(LightningElement) {
    ...
    @api valLimit;isDisableType=false;isDisplayToolType=false;/*1485*/;@api acceptedFormats;toolTipAcceptedFormats;@api valUnlimitChk; @api valExposureBasedOn; @api selectedProgram;@api displayAutoFacFormVal = false;@api displayPCFormVal = false; @api displayLifeFormVal = false;
    ...
    mapCurrency = new Map();
    isCATDeactivated = false; //HRA 29/05/2024 W-0976
    ...
    //BBH 30.10.2024 - Dev ReadOnly W-2163:AUDIT - Create a Read only profile for all platform except Works tab
    renderedCallback(){
        launchDisableMethod(this,'LWC_SA');
    }
    ...
    getSADetail(){
        ...
        .then(result => {
           ...
           //RRA - ticket 1485 - 25092024
            console.log('isCELoadSEReq == ', this.isCELoadSEReq);
            console.log('isModalCopy == ', this.isModalCopy);
            console.log('isLoadPlacementTable__c == ', saObj.isLoadPlacementTable__c);
           if (this.isCELoadSEReq && saObj.isLoadPlacementTable__c){this.isDisableType = true;this.isDisplayToolType=true;} 
            if (this.isCELoadSEReq && saObj.isLoadPlacementTable__c == false){this.isDisableType = false;this.isDisplayToolType=true;} //RRA - ticekt 2182 - 30102024
           if (this.isModalCopy && saObj.isLoadPlacementTable__c){this.isDisableType = false;this.isDisplayToolType=true;} 
           if (this.isModalRenew && saObj.isLoadPlacementTable__c){this.isDisableType = false;this.isDisplayToolType=true;} 
            else if ((profileName == 'System Administrator' || profileName == 'AGRE_System Admin') && saObj.isLoadPlacementTable__c == false){this.isDisableType = false;this.isDisplayToolType=true;}
            ...
            else if(saObj.IsInformCedCom__c == true && saObj.Type__c == '2' && (saObj.InternalStatus__c == 'Refused' || saObj.PortalStatus__c == 'Refused')){this.isBoundDisable = true;}
            ...
            if(saObj.CatCoverage__c && saObj.CatCoverage__c == 2){
            ...
            for(let i = 0; i < this.lstDocuments.length; i++){
                    ...
                    // BBU 20.09.2024 - W-1479 - SA - Size of documents
                    this.lstDocuments[i]['FileSizeRaw'] = this.lstDocuments[i]?.ContentDocument?.ContentSize;
                    this.lstDocuments[i]['FileSize'] = frenchFormatFileSize(this.lstDocuments[i]?.ContentDocument?.ContentSize);
                    // BBU 20.09.2024 - W-1479 - SA - Size of documents
            }
    ...
    handleOnchangeValue(event){
        ...
        if(fieldName == 'Currency__c'){
            ...
            if(value == 1){
                ...
                saObj['FloodLimit__c'] = '';
                saObj['WindstormLimit__c'] = '';
                saObj['EarthquakeLimit__c'] =  '';
                saObj['TerrorismLimit__c'] =  '';
    ...
    handleSaveSubmitSA(event){
        ...
        if(allValid && !this.isTotalAxaShareInvalid){        //MRA W-1251 13/09/2022 
            ...
            if(this.disableAllFields == true && this.isModalEdit == true){
                ...
                this.spinnerSaveSpecialAcceptance = false;
                this.dispatchEvent(new ShowToastEvent({title: 'Error', message: this.label.fileErrorMsg, variant: 'error'}),);
            }
    ...
    handleSendUpdateRemindBtn(){
        ...
        if(this.btnNameclick != 'informCedingCompany' && selectedRequests.length == 0){
            ...
            else if(['send','update','informCedingCompany'].includes(this.btnNameclick) && this.totalFsStyling == 'exceedFileSize'){ //BBU 25.09.24 W-1479
                this.dispatchEvent(new ShowToastEvent({title: 'Error', message:  this.label.fileErrorMsg, variant: 'error',}), );
            }
            else{
