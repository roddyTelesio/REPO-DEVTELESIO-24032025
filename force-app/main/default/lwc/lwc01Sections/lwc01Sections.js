...
import customCssOverides from '@salesforce/resourceUrl/customCssOverides';
...
export default class LWC01_Sections extends NavigationMixin(LightningElement) {
    ...
    connectedCallback(){
        loadStyle(this, customCssOverides);
        ...
        .then(result => {
            ...
            this.data = newResult.map(row => {
                ...
                if(row.Retained_to_lead__c == true){
                    classRetainToLeadData = 'slds-icon slds-icon-text-success greenIconOveride slds-icon_x-small'; // RRA - 1045
                }
                else{
            ...
            this.dataRetainedtoLead = this.data.map(row => {
                ...
                if(row.Retained_to_lead__c == true){
                    retainToLead = false;
                    classRetainToLead = 'slds-icon greenIconOveride slds-icon_x-small'; // RRA - 1045
                }
                else{
                    retainToLead = false;
            ...
            .then(result => {
                if(result.hasOwnProperty('Error') && result.Error){
                    ...
                    this.dataRetainedtoLead = this.data.map(row => {
                        ...
                        if(row.Retained_to_lead__c == true){
                            retainToLead = false;
                            classRetainToLead = 'slds-icon slds-icon-text-success greenIconOveride slds-icon_x-small'; // RRA - 1045
                        }
                        else{
                            retainToLead = false;
                            classRetainToLead = 'slds-icon slds-icon-text-default slds-icon_x-small'; // RRA - 1045
