...
import LightningConfirm from 'lightning/confirm'; //1302
...
import deleteSASelected from '@salesforce/apex/LWC47_SpecialAcceptance.deleteSASelected'; //1302
import chekSASelectedLoadRequest from '@salesforce/apex/LWC47_SpecialAcceptance.chekSASelectedLoadRequest'; //1302
...
// BBU 19.09.2024 - W-1479 - SA - Size of documents
import {frenchFormatFileSize,reCountTotalFileSize} from 'c/lwcUtilityCmp'
...
import messageSAPart1 from '@salesforce/label/c.messageSAPart1';//1302
import messageSAPart2 from '@salesforce/label/c.messageSAPart2';//1302
import messageSAPart3 from '@salesforce/label/c.messageSAPart3';//1302
import messageloadplacementtable from '@salesforce/label/c.messageloadplacementtable';//1302
...
export default class Lwc47SpecialAcceptance extends NavigationMixin(LightningElement) {
    label = {
        ...
        ExtensionNotAuthorizedActorCEAXAXL,
        ...
        ExtensionNotAuthorizedAGRECommunityExternalUser, 
        ExtensionNotAuthorizedAGRECommunityInternalUser, 
        ExtensionNotAuthorizedAGRESystemAdmin,
        ExtensionNotAuthorizedSystemAdministrator,
        messageSAPart1,
        messageSAPart2,
        messageSAPart3,
        messageloadplacementtable
    }
    ...
    @api acceptedFormats; //RRA - ticket 1780 - 18122023
    ...
    toolTipAcceptedFormats; //RRA - ticket 1780 - 19122023
    isCATDeactivated = true; //HRA 29/05/2024 W-0976

    // BBU 19.09.2024 - W-1479 - SA - Size of documents
    totalFileSize = {
        fileSize : 0,
        LIMIT : (50*1024*1024),
        displaySize: '0 Octets',
        viewable : false,
        titleText : "The total size of documents must not exceed 50Mo"
    }
    ...
    formatsFileAccepted(){
        getProfiles()
        .then(resultProfile => {
            if (resultProfile == 'Admin'){
                //this.acceptedFormats = this.fileExtensionsSettings.data.extensionsAcceptedForAdmin__c; if use CS
                this.acceptedFormats = this.label.ExtensionNotAuthorizedSystemAdministrator;
            }else if (resultProfile == 'Admin_AGRE'){
                this.acceptedFormats = this.label.ExtensionNotAuthorizedAGRESystemAdmin;
            }else if (resultProfile == 'Actor_CE'){
                this.acceptedFormats = this.label.ExtensionNotAuthorizedAGRE_Actor_C_E;
            }else if (resultProfile == 'Agre_AXAXL'){
                this.acceptedFormats = this.label.ExtensionNotAuthorizedActorCEAXAXL;
            }else if (resultProfile == 'AgreActor_BasicSalesforce'){
                this.acceptedFormats = this.label.ExtensionNotAuthorizedAGRE_ActorBasicSalesforceAccess;
            }else if (resultProfile == 'AgreActor_BasicSFPlatform'){
                this.acceptedFormats = this.label.ExtensionNotAuthorizedAGRE_ActorBasicSFPlatformAccess	;
            }else if (resultProfile == 'Agre_CommunityExternal'){
                this.acceptedFormats = this.label.ExtensionNotAuthorizedAGRECommunityExternalUser;
            }else if (resultProfile == 'Agre_CommunityInternal'){
                this.acceptedFormats = this.label.ExtensionNotAuthorizedAGRECommunityInternalUser;
            }else if (resultProfile == 'SF_PlatformReadOnly'){
                this.acceptedFormats = 'No extension files';
            }
            let lstAcceptedFormats = this.acceptedFormats.replace(',', ',  ');
            console.log('lstAcceptedFormats lwc14Documents == ', lstAcceptedFormats);
    ...
    connectedCallback() {
        this.formatsFileAccepted();  //RRA - ticket 1780 - 19122023
    ...
    initSAModal(){
        ...
        //CAT Limits
        this.valCatCoverage = '2';
        this.isCATDeactivated = true;  //HRA 29/05/2024 W-0976
    ...
    //*RRA - ticket 1302 - 09072024
    handleDelete (event){
        let selectedSA = this.template.querySelector('lightning-datatable').getSelectedRows();
        let lstIdSA = [];
        if(selectedSA.length == 0){
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Error',
                    message: 'No SA Selected',
                    variant: 'error',
                }),
            );
        }
        else{  
            for (let i=0;i<selectedSA.length;i++){
                lstIdSA.push(selectedSA[i].Id);
            }   
            //Check if in record's SA, at least one request has been sent
                chekSASelectedLoadRequest({ lstSelectedSaId : lstIdSA})
                    .then(checkSASelected => {
                        if ((checkSASelected.messageLoadPlacementTbl != null || checkSASelected.messageLoadPlacementTbl != undefined)){
                            this.dispatchEvent(new ShowToastEvent({title: 'Error', message:checkSASelected.messageLoadPlacementTbl, variant: 'error' }),);
                            return;
                        }else{
                            this.deleteSASelected(selectedSA);
                        }
                    })
                    .catch(error => {
    ...
    async deleteSASelected (selectedSA){
        let messageSA;
        let mapkeyIdSA = [];
        let lstValuesSA = [];
        for (let i=0;i<selectedSA.length;i++){
            this.mapIdSANameSA.set(selectedSA[i].Id, selectedSA[i].SpecialAcceptanceName__c);
        }
        console.log('mapIdSANameSA == ', this.mapIdSANameSA);
            
        this.mapIdSANameSA.forEach((values, keys) => {
            if (this.mapIdSANameSA.size == 1 ){
                messageSA = this.label.messageSAPart1 +' '+ values + this.label.messageSAPart2;
            }else if (this.mapIdSANameSA.size > 1){
                lstValuesSA.push(values)
                messageSA = this.label.messageSAPart3 +' '+ lstValuesSA + ' ' + this.label.messageSAPart2;
            }
            mapkeyIdSA.push(keys);
        });
        
        console.log('keyIdSAMap 22==', mapkeyIdSA);
        console.log('messageSA == ', messageSA);
        const result = await LightningConfirm.open({
            message: messageSA,
            variant: "default", // headerless
            label: "Delete a record Special Acceptance"
        });
        
            //Confirm has been closed
            //result is true if OK was clicked
        if (result) {
            deleteSASelected({ lstSelectedSaId : mapkeyIdSA})
            .then(resultDelete => {
                console.log('resultDelete.messageLoadPlacementTbl ==',resultDelete.messageLoadPlacementTbl);
                if (resultDelete.ErrorDeletion != null){
                    this.dispatchEvent(new ShowToastEvent({title: 'Error', message: resultDelete.ErrorDeletion, variant: 'error' }),);
                }else{
                    this.spinnerSpecialAcceptance = true;
                    if (resultDelete.messageLoadPlacementTbl ==null || resultDelete.messageLoadPlacementTbl ==undefined){
                        if (resultDelete.isDeleted){
                            this.dispatchEvent(new ShowToastEvent({title: 'Success', message: 'SA deleted Successfully', variant: 'success' }),);
                            this.getProgramDetail();
                        }else{
                            this.dispatchEvent(new ShowToastEvent({title: 'Error SA', message:resultDelete.Error, variant: 'error' }),);
                        }
    ...
    handleDeleteDocument(event){
        ...
        this.titleCountDocument = 'My Documents ('+ this.lstDocuments.length +')';    
        reCountTotalFileSize(this.totalFileSize,this.lstDocuments); // BBU 20.09.2024 - W-1479 - SA - Size of documents
    ...
    get totalFsStyling(){
        if(this.totalFileSize.fileSize > this.totalFileSize.LIMIT){
            return 'exceedFileSize';
        }
        return 'fileSizeOK';
    }
    ...
    uploadHelper(){
        ...
        .then(result => {
            ...
            for(let i = 0; i < result.length; i++){
                ...
                // BBU 19.09.2024 - W-1479 - SA - Size of documents
                document['FileSizeRaw'] = result[i]?.ContentDocument?.ContentSize;
                document['FileSize'] = frenchFormatFileSize(result[i]?.ContentDocument?.ContentSize);
                // BBU 19.09.2024 - W-1479 - SA - Size of documents
    ...
    handleSaveSubmitSA(event){
        ...
        if(allValid && !this.isTotalAxaShareInvalid){        //MRA W-1251 13/09/2022 
            ...
            else if(this.totalFsStyling == 'exceedFileSize'){ //BBU 25.09.24 W-1479
            this.spinnerSaveSpecialAcceptance = false;
            this.dispatchEvent(new ShowToastEvent({title: 'Error', message: 'The total size of documents exceeds 50Mo. Your request cannot be sent. Please review the list of documents..', variant: 'error'}),);
            }
    ...
    handleOnchangeValue(event){
        ...
        if(fieldName == 'Currency__c'){
            ...
            //RRA - Ticket correctif Life Form - 31052024
            if(this.displayLifeForm){
            ...
            this.valCatCoverage = value;
             //HRA 29/05/2024 W-0976 -- Start
            if(value == 1){
                this.isCATDeactivated = false;
            }else if (value == 2){
                this.valFloodLimit = '';
                this.valWindstormLimit = '';
                this.valEarthquakeLimit = '';
                this.valTerrorismLimit = '';
                this.isCATDeactivated = true;
            } 
            //HRA 29/05/2024 W-0976 -- End
        }
    ...
    renewSADetail(saRecord){
        //HRA - W-2004
        let saObject = {Id:saRecord.Id,
                        PortalStatus__c:saRecord.PortalStatus__c,
                        SentAgreFromSpecialAcceptance__c:saRecord.SentAgreFromSpecialAcceptance__c,
                        sObjectType:'SpecialAcceptance__c'
                    }
        checkForRenewSA({selectedSa : saRecord})
        .then(result => {
    ...
    handleOnClickBtn(event){
        ...
        if(btnNameclick == 'Bind'){
            ...
            if(saNotSubmissionAgreed == true){
                ...
                this.isBind = true;
                this.isReactivate = false;//RRA - ticket 1450 - 18092024
                this.isDeactivate = false;//RRA - ticket 1450 - 18092024
            ...
            this.isReactivate = false;//RRA - ticket 1450 - 18092024
            this.isBind = false;//RRA - ticket 1450 - 18092024
    ...
    handleAcceptBindDeactReact(event){
        ...
        this.spinnerSpecialAcceptance = true;
        console.log('isDeactivate == ', this.isDeactivate);
        console.log('isReactivate == ', this.isReactivate);
