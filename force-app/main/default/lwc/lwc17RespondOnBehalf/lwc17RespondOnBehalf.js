...
import {launchDisableMethod,secondaryLaunch,isUserReadOnlyProfile} from 'c/lwc65GenericReadOnlyCmp';
...
export default class LWC17_RespondOnBehalf extends NavigationMixin(LightningElement) {
    ...
    getRequestDetails(){
        ...
        .then(result => {
            ...
            if(result.lstRequestAll.length > 0){
                ...
                .then(result => {
                   console.log('###result disableLeadPlacementInfo == ', result);
                    if(requestPhase != '3'){
                        //Disable Lead and Placement Respond on Behalf if phase is not quote
                        this.disableLeadPlacementInfo = result;
                    }
                  console.log('###result disableLeadPlacementInfo == ', this.disableLeadPlacementInfo);
                })
                .catch(error => {
                ...
                let requestPhase = result.requestPhaseType;
                console.log('###result requestPhase == ', requestPhase);
                ...
                console.log('###result else disableLeadPlacementInfo == ', this.disableLeadPlacementInfo);
            }
    ...
    setTypeOfTreatyQuoteValue(rowSection){
        ...
        //RRA - ticket 2204 - 09/12/2025
        if(typeOfTreaty == '3'){
            rowSection['isTreatyTypeQS'] = true;
            rowSection['isTreatyTypeSurplus'] = false;
        ...
        console.log('isTreatyTypeQS lwc17 == ',  this.isTreatyQSType);
          console.log('isTreatyTypeSurplus lwc17 == ', this.isTreatySPType);
        ...
        if(this.isPhaseTypeLead == true){
            if(rowSection.OverridingCommission__c == null || rowSection.OverridingCommission__c == ''){
              ...
              console.log('typeOfTreaty == ', typeOfTreaty);
                console.log('typeOfQuote == ', typeOfQuote);
            //QS + (FlatCommission or VariableCommission or RiskPremiumBasis)
            if((typeOfTreaty == '3' || typeOfTreaty == '5') && (typeOfQuote == '5' || typeOfQuote == '6' || typeOfQuote == '9')){//RRA - Ticket 2248 - 30122024
                let totalEPI = 0;
                let cessionPerc = 0;
                ...
                console.log('totalEPI == ', totalEPI);
                console.log('cessionPerc == ', cessionPerc);
            ...
            else if((typeOfTreaty == '3' || typeOfTreaty == '4' || typeOfTreaty == '5') && typeOfQuote == '7'){ //RRA - Ticket 2248 - 30122024
                let totalEPI = 0;
                if(rowSection.Section__r.TotalEPI__c != undefined){
                    totalEPI = rowSection.Section__r.TotalEPI__c;
                }
    ...
    handleCloseModal(){//'+this.selectedTreatyId+ => replace by null' RRA ticket 585 - 14032023
        console.log('here bbh '+isUserReadOnlyProfile())
        fireEvent(this.pageRef, 'refreshReq', 'refresh');
        if(isUserReadOnlyProfile()){//BBH READONLY PROFILE W-2163
            window.history.back();
        }
        else{
