...
//BBH 18.06.2024 - CAT DATA - W-2045
import {navigateToRecordViewPage,documentTypeRedirect,docTypeToExport} from 'c/lwcUtilityCmp'
...
export default class LWC14_Documents extends NavigationMixin(LightningElement) {
    ...
    @api valueProgram;
    @api programOptions;
    @api acceptedFormats;//RRA - ticket 1780 - 19122023
    @track groupTypeOpt = [];
    ...
    // BBU W-2135: DOCUMENTS - Impossible to delete a doc - HANDLE ERRORS
    inProgress = false;
    ...
    handleOpenNewDocumentModal(){
        ...
        // BBH RESET VALUE 04.07.2024
        this.massAssignDocType.selectedValue = '';
    }
    ...
    uploadHelper(){
        ...
        for(var i = 0; i < this.uploadedDoc.length; i++){
            ...
            .then(result => {
                ...
                document['DocumentType'] = this.massAssignDocType.selectedValue;
                document['Phase'] = '5';
                document['PhaseOptions'] = this.phaseOpt;
                document['Checked'] = true; //BBH 01.07.2024 - W-1092 SET CHECKBOX TRUE
                document['Id'] = result.Id;
                document['DocumentTypeOpt'] = [];
                document['isGroupEmpty'] = true;
                document['Viewable'] = true;

                // BBH 27.06 CAT DATA SHARE W-1092
                ...
                this.lstDocuments = this.sortData('CreatedDate', 'desc', this.lstDocuments); //BBH 01.07.2024 - W-1092
            })
            .catch(error => {
                this.dispatchEvent(new ShowToastEvent({title: 'Error', message: this.label.errorMsg, variant: 'error'}), );
            });
    ...
    handleViewProgram(){
        ...
        console.log('agreementRtSelected == ', this.agreementRtSelected);
    ...
    // BBH CAT FILES 24.06.2024 - W -2045  - START
    lstDocIdsToExport = [];
    exportExcel = false;
    showExportSharingBtn = false;
    handleSharingInfo(){
    ...
    // BBH CAT FILES 24.06.2024 - W -2045 - END
    ...
    getDocumentValue(){
        ...
        .then(result => {
            ...
            for(var i = 0; i < lstProgramDocuments.length; i++){
                ...
                // BBH 18.06.2024 W-2047
                document['ContentDocumentId'] = lstProgramDocuments[i].ContentDocumentId;
                if(docTypeToExport.includes(document['DocumentType'])){
                    this.lstDocIdsToExport.push(document['Id']); //con version id
                    this.showExportSharingBtn = true;
                }
                // BBH 18.06.2024 W-2045

                //BBH 18.06.2024 - CAT DATA - W-2045
                document['Redirect'] = false;
                if(documentTypeRedirect.includes(document['DocumentType'])){
                    document['Redirect'] = true;
                }
                //BBH 18.06.2024 - CAT DATA - W-2045

                //BBH 27.06.2024 - CAT DATA W-1092
                document['CreatedDate'] = lstProgramDocuments[i]?.CreatedDate;
                //BBH 27.06.2024 - CAT DATA W-1092
    ...
    //BBH 18.06.2024 - CAT DATA - W-2045
    navToFiles(event){
        navigateToRecordViewPage(this,event.currentTarget.dataset.id,'ContentDocument','Redirect')
    }

    // BBH 01.07.2024 W-1092 
    @track enableComboboxMassDT = false;
    get disableCombobox() {
        return !this.enableComboboxMassDT;
    }
    // BBH 01.07.2024 W-1092 
    ...
    handleCheckboxChange(event) {
        this.enableComboboxMassDT = event.target.checked;

        if(!this.enableComboboxMassDT){
            this.massAssignDocType.selectedValue = '';
        }
    }
    // BBH 01.07.2024 W-1092
    ...
    // BBH 01.07.2024 W-1092
    get massDocTypeOpt(){
        if(!this.massAssignDocType.picklistVals || this.massAssignDocType.picklistVals.length == 0){
            this.massAssignDocType.picklistVals = [...this.getDocTypeByGroupType(this.massAssignDocType.groupType)];
        }
        return this.massAssignDocType.picklistVals;
    }
    // BBH 01.07.2024 W-1092

    //PLEASE REUSE THIS NEXT TIME INSTEAD OF COPYING THE SAME CODE EVERYWHERE
    getDocTypeByGroupType(grpType){
        let pickVals = []
        this.totalDependentDocumentTypeValues.forEach(groupTypeVal => {
            if(groupTypeVal.validFor[0] === grpType){
                pickVals.push({
                    label : groupTypeVal.label,
                    value : groupTypeVal.value
                })
    ...
    // BBH 01.07.2024 W-1092
    handleSelectMassDocType(event){
        this.massAssignDocType.selectedValue = event.target.value;
    }
    // BBH 01.07.2024 W-1092

    // BBH 01.07.2024 W-1092
    get showMassAssignOpt(){
        if(this.documentNames && (this.documentNames.match(/;/g) || []).length>1){
            return true;
        }
        this.enableComboboxMassDT = false;
        return false;
    }
    // BBH 01.07.2024 W-1092
