...
export default class Lwc49SendSaRequest extends NavigationMixin(LightningElement) {
    ...
    sendUpdateSaRequest() {
        ...
        for (let i = 0; i < this.lstPoolsData.length; i++) {;
            //RRA - ticket 1264 - 28062024
            if (this.lstPoolsData[i]['SA_Email__c'] !== undefined){
                poolToEmail = this.lstPoolsData[i]['Id'] + '|' + this.lstPoolsData[i]['SA_Email__c'];
                lstPoolIdToEmailStr.push(poolToEmail);
            }else{
                isMailPoolExist = false;
            }
        };;
        //AMI 16/05/22: W-1101
        ...
        if (typeof selectedContactRecords !== undefined && selectedContactRecords.length > 0) {
        ...
        }else if (isMailPoolExist == false){ 
            //No BAL Email Exists on Pool => Display Message noContactSelected                                                         
             //RRA - ticket 1264 - 28062024
            this.dispatchEvent(new ShowToastEvent({
                title: 'Error',
            ...
            //BAL Email Exists on Pool => Not Display Message noContactSelected
             //RRA - ticket 1264 - 28062024
            this.spinnerRequest = false;
        }
        console.log('lstPoolIdToEmailStr == ' ,lstPoolIdToEmailStr);
        console.log('finalConListToProcess == ' ,this.finalConListToProcess);
        console.log('dataContact == ' ,this.dataContact);
         //RRA - ticket 1264 - 28062024
        if (lstPoolIdToEmailStr.length>0 || this.finalConListToProcess.length>0 || this.finalConListToProcess.length == 0){
            sendUpdateSaRequest({
                lstSaRequests: this.saRequest,
                lstAccountContact: this.finalConListToProcess,
                lstAccountContactForSharingSA: this.dataContact, //RRA - Ticket HotFix 2118 - 06092024
