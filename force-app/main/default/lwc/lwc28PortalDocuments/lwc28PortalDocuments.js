...
// BBH 03.07.2024 UTILITY METHOD 
import {formatFileSize,sortObjectsByFields} from 'c/lwcUtilityCmp'
...
export default class LWC28_PortalDocuments extends NavigationMixin(LightningElement) {
    ...
    toolTipAcceptedFormats; //RRA - ticket 1780 - 21122023
    ...
    handleSaveUploadDoc(){
        this.notifyProcessToParent(true); //BBU 13.11.24 - HOTFIX W-2186 : HOTFIX - Reinsurer document not available on CE portal
        ...
        .then(result => { 
            ...
            .then(result => {
                if (result.Success = 'Success'){
                    ...
                    this.notifyProcessToParent(false); //BB 13.11.24 W-2186 HOTFIX - Reinsurer document not available on CE portal
    ...
    //BBU 13.11.24 - W-2186 HOTFIX - Reinsurer document not available on CE portal
    notifyProcessToParent(spinner){
        const selectedEvent = new CustomEvent("processing", { detail: {
            ...
            let brokerExist = this.selectedBroker;
             ...
             .then(result => {
                ...
                .then(result => {
                    if(result.hasOwnProperty('Error') && result.Error){
                        ...
                        if(result.lstConvInternal != undefined){
                            ...
                            for(let i = 0; i < lstProgramDocumentsInternal.length; i++){
                                    ...
                                    document['DocumentTypeLabel'] =lstProgramDocumentsInternal[i].CLM_Name__c ? 'Acceptance – Special Conditions' : lstProgramDocumentsInternal[i].docTypeLabel; //HOTFIX W-2127 - External Portal - Error message on Documents button
                                    //BBH 03.07.2024 - Display file size
                                    document['FileSize'] = formatFileSize(lstProgramDocumentsInternal[i]?.ContentDocument?.ContentSize);
                                    document['Checked'] = false;
                            ...
                            console.log('lstNewDocumentsInternal ',lstNewDocumentsInternal);
                    ...
                    if(this.lstAXADocuments && this.lstAXADocuments.length > 0){
                        this.lstAXADocuments = sortObjectsByFields(this.lstAXADocuments, 'DocumentTypeLabel', 'Name');
                        // W-2127 - External Portal - Error message on Documents button
                        const phaseOrder = ['Signing','Placement', 'Lead','Quote','Conditions', 'All'];
                        this.lstAXADocuments.sort((a, b) => {
    ...
    //     this.downloadDocs(this.lstAXADocuments);
        
    // }

    //BBH : METHOD CENTRALISÉ
    handleClick(event){
        let src = event.target.name;
        if(src=="all"){
            this.downloadDocs(this.lstAXADocuments,'records'); 
        }
        if(src="selected"){
            console.log('pass selected doc Ids');
            this.downloadDocs(this.lstSelectedDocIds,'ids');
        }

    }

    downloadDocs(documents,objType){
        //link to downlaod all possible docs
        let DocsUrl = '../sfc/servlet.shepherd/version/download';

        //loop in retrieved and updated  docs
        if(objType == 'records'){
            documents.map(ele => {
                //build new doc url to enable mass download
                DocsUrl = DocsUrl.concat('/',ele.Id);
            });
        }
        else{
            DocsUrl+= '/'+documents.join('/');
        }

        //add context after successfull url build
        DocsUrl = DocsUrl.concat('?operationContext=S1');
    ...
    lstSelectedDocIds=[];
    disableSelectedDoc = true;
    handleChange(event){
        const typeAttributeValue = event.target.type;
        ...
        switch (typeAttributeValue) {
            case 'checkbox':
                let srcName = event.target.name;
                if(srcName === 'CheckAll'){
                    //logic for checkAll
                    this.lstSelectedDocIds = [];
                    this.lstAXADocuments.forEach(doc => {
                        doc['Checked'] = val;
                        this.lstSelectedDocIds.push(doc.Id);
                    });
                    ...
                    if(val){
                        this.lstSelectedDocIds.push(srcName);
                    }
                    else{
                        //remove id
                        if(this.lstSelectedDocIds.includes(srcName)){
                            const index = this.lstSelectedDocIds.indexOf(srcName);
                            if (index > -1) {
                                this.lstSelectedDocIds.splice(index, 1);
                            }
                        }
                    }
                }
                this.disableDownloadAll = !this.lstSelectedDocIds.length == 0;
                this.disableSelectedDoc = this.lstSelectedDocIds.length == 0;
