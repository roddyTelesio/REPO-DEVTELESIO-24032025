...
import getCocessionFields from '@salesforce/apex/LWC01_NewTreaty.getCocessionFields';//RRA - ticket 1424 - 18072024 
import deleteRequestAttachedOnPool from '@salesforce/apex/LWC01_NewTreaty.deleteRequestAttachedOnPool'; //RRA VERSIONING - Delete Request associated to Pool - 04102024
...
import CESSIONSHARE_FIELD from '@salesforce/schema/Treaty__c.CessionShare__c'; //RRA - ticket 1966 -14032024
...
import CessionShare from '@salesforce/label/c.CessionShare';
...
import CessionError from '@salesforce/label/c.CessionError'; //RRA - ticket 1966 - 14032024
...
import PlacementShareTypeHelpText from '@salesforce/label/c.PlacementShareTypeHelpText'; //RRA - ticket 1966 - 14032024
import CessionShareHelptext from '@salesforce/label/c.CessionShareHelptext';
...
import axaSAAndaxaXL from '@salesforce/label/c.axaSAAndaxaXL';//RRA - ticket 1424 - 18072024
import axaSAAndAxaXLCYBER from '@salesforce/label/c.axaSAAndAxaXLCYBER';//RRA - ticket 1424 - 18072024
import CocessionFiedsValueMissing from '@salesforce/label/c.CocessionFiedsValueMissing';//142
...
export default class LWC01_NewTreaty extends NavigationMixin(LightningElement) {
    label = {
        ...
        LossAttachment,
        PlacementShare,
        CessionShare, //RRA - ticket 1966 - 14032024
        Deductions,
        NotManagedByAgre,
        DeductionsPerc,
        LossDeposit,
        ...
        TotalPoolError,
        PlacementError,
        CessionError, //RRA - ticket 1966 -15032024
        DeductError,
        TreatyUpdate,
        TreatyCreated,
        FormEntriesInvalid,
        ...
        CessionShareHelptext, //RRA - ticket 1966 - 14032024
        twoDpErrorMessage,
        cocessionFieldsTotal,
        errorMsg,
        axaSAAndAxaXLCYBER,//RRA - ticket 1424 - 18072024
        axaSAAndaxaXL,//RRA - ticket 1424 - 18072024
        CocessionFiedsValueMissing //RRA - ticket 1424 - 18072024
    }
    ...
    nameURL;
    nameURLPage;
    isPlacementShareVisible = false; //RRA - ticket 1966 - 14032024
    isCessionShareVisible = false; //RRA - ticket 1966 - 14032024
    isRequiredPlacementShare = false;//RRA - ticket 1966 - 14032024
    isRequiredCessionShare = false;//RRA - ticket 1966 - 14032024
    disableFieldCocession = false;//RRA - ticket 1424 - 17072024
    ltaOpt;
    LtaPattern;
    ltaPatternOpt;
    TacitRenewalOpt;
    ...
    cessionShareEmpty = false; //RRA - ticket 1966 - 14032024
    ...
    setExistingAccountIdSections = new Set();
    disableTreatyTypeField = false;
    disablePlacementShare = false;
    disableCessionShare = false; //RRA - ticket 1933 - 14032024
    ...
    @track objTreaty = {
        ...
        CessionShare__c : CESSIONSHARE_FIELD, //RRA - ticket 1966 - 14032024
    ...
    connectedCallback(){
        ...
        if(this.pageRef.attributes.apiName != null && this.pageRef.attributes.apiName != undefined){
            this.nameURL = this.pageRef.attributes.apiName;
        }
        
        if(this.pageRef.attributes.pageName != null && this.pageRef.attributes.pageName != undefined){
            this.nameURLPage = this.pageRef.attributes.pageName;
        }
        
        console.log('nameURL == ' , this.nameURL);
        console.log('nameURLPage == ' , this.nameURLPage);
        ...
        if(this.isTreatyCopy == true || this.isTreatyEdit == true){
            ...
            .then(result => {
                ...
                if(this.isTreatyEdit == true){
                    ...
                    this.disableCessionShare = result['CessionShare'];
                }
            })
            .catch(error => {
                this.error = error;
            ...
            if(this.isTreatyEdit == true){
                ...
                .then(result => {
                    ...
                    this.objTreaty.CessionShare__c = result[0].CessionShare__c;// RRA - ticket 1966 - 27032024
                    console.log('Principal_Ceding_Company__c == ', result[0].Principal_Ceding_Company__c);
                    //RRA - ticket 1424 - 17072024
                    if (result[0].Principal_Ceding_Company__c ==  this.label.axaSAAndaxaXL || result[0].Principal_Ceding_Company__c ==  this.label.axaSAAndAxaXLCYBER){
                        ...
                        console.log('ok 1424');
                        this.objTreaty.AGRe_Cession__c = null;
                        this.objTreaty.Ceded_Re_Cession__c = null;
                        this.objTreaty.GI_Cession__c = null;
                        this.disableFieldCocession = true;
                    }
            ...
            }else if (this.selectedTreaty == 'AXA XL QS'){
                this.selectedTreaty = '5';
            }
            
             //RRA - ticket 1966 - 14032024
             if (this.selectedTreaty == '5'){
                // CessionShare Field
                this.isCessionShareVisible = true; 
                this.isPlacementShareVisible = false;
                this.isRequiredPlacementShare = false;
                this.isRequiredCessionShare = true;
            }else{
                // PlacementShare Field
                this.isPlacementShareVisible = true;  
                this.isCessionShareVisible = false; 
                this.isRequiredPlacementShare = true;
                this.isRequiredCessionShare = false;
            }


            //QS or Surplus or AXA XL QS -> Premium Editable
            //SL or XL -> Premium Disabled
            if(this.selectedTreaty == '3' || this.selectedTreaty == '4' || this.selectedTreaty == '5'){//1966
            ...
            this.objTreaty.CessionShare__c = this.selectedRowTreaty.CessionShare__c; //RRA - ticket 1966 - 14032024
            ...
            .then(result => {
                ...
                //RRA - ticket 1424 - 17072024
                if (result[0].TECH_PCCName__c === this.label.axaSAAndaxaXL || result[0].TECH_PCCName__c === this.label.axaSAAndAxaXLCYBER){
                    console.log('Equal');
                }else{
                    this.objTreaty.AGRe_Cession__c = null;
                    this.objTreaty.Ceded_Re_Cession__c = null;
    ...
    generatePicklistValues(){
        ...
        if(this.selectedTreaty){
            ...
            this.totalDependentReinsuranceValues.forEach(treatyValues => {
                ...
                }else if (treatyValues.validFor[1] === this.controlReinsuranceValues[this.selectedTreaty]){//RRA - ticket 1966 - 15032024
                    dependValues.push({
                        label : treatyValues.label,
                        value : treatyValues.value
                    })
    ...
    handleAddPool(event){
        ...
        console.log('lstPoolchecked == ', lstPoolchecked);
        dataUpdPools = this.getUniqueData(lstPoolchecked,'Id');
        console.log('dataUpdPools == ', dataUpdPools);
    ...
    handleDeletePool(){
        let selectedPool = [];
        let selectedIdPool = []; //RRA VERSIONING - Delete Request associated to Pool
        for(let i = 0; i < this.dataPools.length; i++){
            if(this.dataPools[i].Checked == true){
                selectedPool.push(this.dataPools[i]);
                selectedIdPool.push(this.dataPools[i].Id); //RRA VERSIONING - Delete Request associated to Pool
            }
        }
        console.log('selectedIdPool == ', selectedIdPool); //
        console.log('selectedRowTreaty.id == ', this.selectedRowTreaty.Id);
        //RRA VERSIONING - Delete Request associated to Pool - 04102024
        if (selectedIdPool.length>0){
            deleteRequestAttachedOnPool({lstPoolId : selectedIdPool, treatyId : this.selectedRowTreaty.Id})
            .then(data => {
                console.log('Delete Request associated successfully');
            })
            .catch(error => {
                this.error = error;
                this.data = undefined;
    ...
    handleSaveTreaty(){
        ...
        this.objTreaty.CessionShare__c = parseFloat(dataInput.CessionShare); //RRA - ticket 1966 - 14032024
        ...
        || (this.objTreaty.PremiumDeposit__c == null && this.premiumRequired == true) 
        || ((this.objTreaty.PlacementShare_Perc__c == undefined || isNaN(this.objTreaty.PlacementShare_Perc__c) || this.objTreaty.CessionShare__c == NaN) && this.isPlacementShareVisible == true && this.isCessionShareVisible == false)
        || ((this.objTreaty.CessionShare__c == undefined || isNaN(this.objTreaty.CessionShare__c || this.objTreaty.CessionShare__c == NaN )) && this.isCessionShareVisible == true && this.isPlacementShareVisible == false) //RRA - ticket 1966 - 14032024
        ...
        || (this.objTreaty.TECH_LTA__c == '1' && this.objTreaty.LTAPattern__c == '')){
            ...
            if(this.objTreaty.CessionShare__c == undefined || isNaN(this.objTreaty.CessionShare__c)){
                this.cessionShareEmpty = true;
            }
            ...
            if (this.dataPools != undefined){
                for (var i = 0; i < this.dataPools.length; i++){
                    poolSaveData.push(this.dataPools[i].Id + ';' + parseFloat(this.dataPools[i].ShareRate__c) + ';' + this.dataPools[i].TECH_Temp_KeyRenew__c); //RRA - Ticket 2244 - 16/01/2025
                }
            }
            ...
            if(result == true){
                ...
                if(this.isTreatyCopy == true && oldTreatyName == dataInput.TreatyName.trim()){
                ...
                else if(this.objTreaty.PlacementShare_Perc__c != "" && this.objTreaty.PlacementShare_Perc__c > 100 && this.isPlacementShareVisible == true && this.isCessionShareVisible == false) { //RRA - ticket 1966 - 14032024
                    this.dispatchEvent(
                        new ShowToastEvent({
                            title: 'Error',
                ...
                else if(this.objTreaty.CessionShare__c != "" && this.objTreaty.CessionShare__c > 100 && this.isPlacementShareVisible == false && this.isCessionShareVisible == true) { //RRA - ticket 1966 - 14032024
                    this.dispatchEvent(
                        new ShowToastEvent({
                            title: 'Error',
                            message: this.label.CessionError,
                            variant: 'error'
                        }),
                    );
                    this.spinnerTreaty = false;
                    ...
                    console.log('data selectedTreatyId == ', this.selectedTreatyId);
                    console.log('data programId == ', this.programId);
                    console.log('data sumCession == ', sumCession);
                    console.log('data AGReCession == ', parseFloat(dataInput.AGReCession));
                    console.log('data CededReCession == ', parseFloat(dataInput.CededReCession));
                    console.log('data GICession == ', parseFloat(dataInput.GICession));
                    //RRA - ticket 1424 - 17072024
                    console.log('isTreatyEdit == ', this.isTreatyEdit);
                    
                    //if(this.isTreatyEdit == false){
                        // if (this.selectedTreatyId != null && this.selectedTreatyId != undefined){
                       //RRA - ticket 2133 - 30092024
                       getCocessionFields({treatyId : this.selectedTreatyId, programId : this.programId})
                       .then(data => {
                           console.log('data getCocessionFields == ', data);
                           //this.spinnerTreaty = false;
                           if (data.length > 0){
                               //Edit and Creation
                               if (data[0].Principal_Ceding_Company__c ===  this.label.axaSAAndaxaXL || data[0].Principal_Ceding_Company__c ===  this.label.axaSAAndAxaXLCYBER){
                                   if (isNaN(parseFloat(dataInput.AGReCession)) || isNaN(parseFloat(dataInput.CededReCession)) || isNaN(parseFloat(dataInput.GICession))){
                                       this.dispatchEvent(new ShowToastEvent({title: 'Error', message:this.label.CocessionFiedsValueMissing, variant: 'error'}),);
                                       this.spinnerTreaty = false;
                                   }else {
                                       if(sumCession.toPrecision(6) != 100){
                                           this.dispatchEvent(new ShowToastEvent({title: 'Error', message: this.label.cocessionFieldsTotal, variant: 'error' }),);
                                           this.spinnerTreaty = false;
                                       }else{
                                           if (this.selectedTreatyId != null && this.selectedTreatyId != undefined){
                                               this.saveTreaty (this.objTreaty, lstAccountIdCovCedingCom, poolSaveData, this.selectedTreatyId, this.isTreatyEdit);
                                           }else{
                                               this.saveTreaty (this.objTreaty, lstAccountIdCovCedingCom, poolSaveData, null, this.isTreatyEdit);
                                           }
                                ...
                                console.log('objTreaty == ', this.objTreaty);
                                this.saveTreaty (this.objTreaty, lstAccountIdCovCedingCom, poolSaveData, null, this.isTreatyEdit);
                           }
                       })
                       .catch(error => {
      ...
      //RRA - ticket 1424 - 17072024
    saveTreaty (objTreaty, lstAccIdCovCedCom, lstPools, treatyId, editTreaty){
        ...
        return validSoFar && inputCmp.checkValidity();
            ...
            if(allValid){
                saveTreatyRecord({ objectTreaty : objTreaty, lstAccIdCovCedCom : lstAccIdCovCedCom, lstPools : lstPools, treatyId : treatyId, editTreaty : editTreaty})
                .then(result => {
                    console.log('result saveTreatyRecord== ', result);
                    this.objTreaty = {};
                    this.dataInput = {};
                    this.data = [];
                    ...
                    if(this.isTreatyEdit){
                        ...
                        fireEvent(this.pageRef, 'saveCondProgramTP', this.programId);// MRA W-0779 18/08/2022
                        fireEvent(this.pageRef, 'isSaveProgramTPFromTreaty', true); //RRA - 1372 - 03092024
                        this.spinnerTreaty = false;
                    }
    ...
    getCovCedComByProgramId(programId){
        getCovCedComByProgramId({selectedProgramId: programId, isEdit : this.isTreatyEdit})
        .then(result => {
            this.data = result;
            if(this.data.length == 0 && this.isTreatyEdit == false){
