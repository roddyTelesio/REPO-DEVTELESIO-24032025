({
	initComponent : function(component,event,helper) {
        ...
        */
        ...
        console.error('initComponent: END');
    },
    navigate2target : function(component,event,helper) {
        console.error('navigate2target: START');
        
        let outputVar = event.getParam("outputVariables");
		console.log('navigate2target: outputVar fetched',outputVar);
		...
		if (!targetId) {
			console.warn('navigate2target: no targetId found, closing current tab');

			var workspaceAPI = component.find("workspaceUtil");
		...
		.then(function(isConsole) {
			...
			if (isConsole) {
				...
				.then(function(tabId) {
					...
					let openEvent = $A.get("e.c:GRC_TabOpen_EVT");
					openEvent.setParams({
						"sourceId" : tabId,
		...
		console.error('navigate2target: checking if in console mode');  
    }
})